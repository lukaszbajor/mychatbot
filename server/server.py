...
async def connect(sid, environ):
    ...
    async def session_request(sid, data):
        ...
        sessions[sid] = {"conversation_id": "", "rasa_sio": AsyncClient()}
        client = sessions[sid]

        try:
            if CONFIG["RASA_SOCKET_PATH"]:
                await client["rasa_sio"].connect(
                    url=CONFIG["RASA_SOCKET_URL"],
                    socketio_path=CONFIG["RASA_SOCKET_PATH"],
                )
            else:
                await client["rasa_sio"].connect(url=CONFIG["RASA_SOCKET_URL"])
        except Exception as e:
            logging.error(f"Rasa connection error {e}")
            return
        await client["rasa_sio"].emit(
            "session_request", {"session_id": conversation_id}
        )
        logging.info(
            f"client {sid} send session_requewt to rasa, is_connected: {client['rasa_sio'].connected}"
        )

        @client["rasa_sio"].on("connect")
        async def connect():
            logging.info("Rasa connect")

        @client["rasa_sio"].on("session_confirm")
    ...
    async def user_uttered(sid, data):
        ...
        if not user_message:
            ...
            return
        if not client["rasa_sio"].connected:
            logging.error(f"Rasa client {sid} is not connected")
            return
        await client["rasa_sio"].emit("user_uttered", data)
    ...
    async def disconnect(sid):
        ...
        await client["rasa_sio"].disconnect()
        if sid in sessions:
    ...
    async def submit_survey(sid, data):
        ...
        comment = next(
            (item["value"] for item in data if item["input"] == "Komentarz"), ""
        )
