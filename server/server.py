...
#         # Sprawdzenie formatu danych w session_confirm
#         logging.info(f"Received data in session_confirm: {data}")

#         # Obsługa różnych formatów danych
#         if isinstance(data, str):
#             conversation_id = data  # jeśli to string, traktujemy jako conversation_id
#         elif isinstance(data, dict):
#             conversation_id = data.get(
#                 "conversation_id"
#             )  # jeśli to słownik, używamy .get()
#         else:
...
#             sessions[sid] = conversation_id  # Zapisujemy session_id dla tego klienta
#             await sio.emit(
...
#     @rasa_sio.on("bot_uttered")
#     async def bot_uttered(data):
#         logging.info(f"bot_uttered: {data}")
#         await sio.emit("bot_uttered", data, to=sid)
...
# # Krok 6: Wysyłanie odpowiedzi po wypełnieniu ankiety
# @sio.on("submit_survey")
# async def submit_survey(sid, data):
#     rating = next(
#         (item["value"] for item in data if item["input"] == "Ocena bota"), None
...
# # Krok 4: Obsługa odpowiedzi od Rasa
# @rasa_sio.on("bot_uttered")
# async def bot_uttered(data):
#     logging.info(f"Bot response received: {data}")
#     # Przekazywanie odpowiedzi od Rasa do klienta
#     sid = sessions.get(data.get("session_id"))
#     if sid:
#         logging.info(f"Sending bot response to client SID={sid}")
#         await sio.emit("bot_uttered", data, to=sid)
#     else:
#         logging.error("Session not found for bot response")
...
# # else:
# #     logging.error(f"Session not found for bot response, session_id={session_id}")
...
CONFIG = {
    ...
    "SERVER_PORT": 5000,
    "RASA_SOCKET_URL": "http://localhost:5005",
    "RASA_SOCKET_PATH": "",  # Ścieżka WebSocket do Rasa
}
